...
KBC is a register-based bytecode. The interpreter dispatches **112
opcodes** via a `while`/`when` loop. Each opcode is one byte; operands
follow.
...
tag-byte-prefixed payload (see [Compose Tokens](compose-tokens.md) for
...
Next: [`.ktx` Bundle Format →](ktx-bundle-format.md)
