...
KBC bundles can't open sockets directly, that would defeat the
capability sandbox. Instead, you call **HTTP capabilities** that the
host registers against Ktor + OkHttp (built-in) or against Retrofit /
your own client (custom).
    ...
    fun load(userId: Long) {
        viewModelScope.launch {
            ...
            try {
                ...
                _uiState.update { it.copy(user = json, loading = false) }
            } catch (e: Exception) {
    ...
    init {
        viewModelScope.launch {
            sseSubscribe("https://api.example.com/ticker/AAPL")
                .collect { line -> _price.value = line }
        }
    }
}
```
