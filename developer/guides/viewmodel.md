...
KBC ViewModels are real Android `ViewModel`s — they survive
configuration change, scope coroutines to `viewModelScope`, and persist a
filtered state subset across process death via `SavedStateHandle`.
...
compiler can resolve them (see [Custom Capability](custom-capability.md)
...
DAO/Repository. See [Room guide](room.md).
...
Next: [Coroutines & Flow →](coroutines-flow.md)
