...
// OTA delivery motion — a self-contained port of the "OTA Update" scene
// (OTA Mobile App Update/ota-scene.jsx + animations.jsx). The editor chrome
// (playback bar, scrubber, keyboard, localStorage, font-inlining, video export)
// is stripped; what's left is the looping scene, responsively scaled from its
// native 1920×1080, paused when offscreen, and frozen to a representative
...
// ── timeline primitives (ported verbatim from animations.jsx) ────────────────
const Easing = {
  linear: (t) => t,
  easeInQuad: (t) => t * t,
  easeOutQuad: (t) => t * (2 - t),
...
const TimelineContext = createContext(0);
const useTime = () => useContext(TimelineContext);

// ── palette (mapped onto site fonts; keeps the Ketoy green) ──────────────────
const C = {
  ink: '#e9fff4',
...
const DURATION = 15;
// A representative still for reduced-motion: patch landed, app updated, toast +
// tagline both visible.
const STATIC_TIME = 10.5;
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden' }}>
      <div style={{
        ...
        background: 'radial-gradient(120% 90% at 50% 22%, #34c479 0%, #1f8a54 34%, #114a30 62%, #0a2418 82%, #05130d 100%)',
      }} />
      <div style={{
      ...
      <svg width="1920" height="1080" viewBox="0 0 1920 1080" style={{ position: 'absolute', inset: 0 }}>
        {TRACES.map((d, i) => (
          <path key={i} d={d} fill="none" stroke="rgba(255,255,255,0.13)" strokeWidth="3.4"
        ...
        {[0, 2].map((idx, k) => {
          ...
          return (
            <path key={'p' + k} d={TRACES[idx]} fill="none" stroke={C.violet} strokeWidth="4.6"
              strokeLinecap="round" strokeDasharray={`${dash} ${gap}`} strokeDashoffset={off}
      ...
      {DOTS.map(([x, y], i) => {
        ...
        return (
          <div key={i} style={{
            ...
            opacity: 0.35 + pulse * 0.5, boxShadow: `0 0 ${8 + pulse * 10}px rgba(255,255,255,0.8)`,
          }} />
        );
      })}
    </div>
...
function Terminal() {
  ...
  return (
    <div style={{
      ...
      padding: '26px 34px 30px', borderRadius: 20,
      ...
      backdropFilter: 'blur(12px)', fontFamily: C.mono,
    }}>
      ...
      {LOG.map((l, i) => {
        ...
        return (
          <div key={i} style={{
            display: 'flex', gap: 14, alignItems: 'baseline', fontSize: 21, lineHeight: 1.75,
            opacity: lo, transform: `translateX(${(1 - e) * 10}px)`,
            color: isRocket ? C.ink : 'rgba(234,231,255,0.9)', fontWeight: isRocket ? 600 : 400,
          }}>
...
function Beam() {
  ...
  return (
    <div style={{
      position: 'absolute', left: BEAM_X - 2, top: BEAM_Y0, width: 4, height: h, opacity: op,
      background: `linear-gradient(${C.glow}, rgba(29,183,101,0.05))`,
      boxShadow: '0 0 18px rgba(29,183,101,0.7)', borderRadius: 2,
    }} />
  );
}
...
function Packet() {
  ...
  return (
    <div style={{
      ...
      transform: `translate(-50%,-50%) scale(${scale}) rotate(${spin}deg)`, opacity: op,
    }}>
      <div style={{
...
function VersionBadge({ updated }) {
  ...
  return (
    <div style={{ fontFamily: C.mono, fontSize: 14, marginTop: 3, color: updated ? C.green : C.inkDim }}>
...
function Phone() {
  ...
  const LAND = 7.0;
  const flash = clamp((t - LAND) / 0.5, 0, 1);
  ...
  return (
    <div style={{
      ...
      transform: `translate(-50%, ${(1 - appear) * 40 + jolt}px)`, opacity: appear,
    }}>
      <div style={{
        position: 'relative', width: W, height: H, borderRadius: '52px 52px 0 0',
        background: '#0e0d16', border: '10px solid #1b1a26', borderBottom: 'none',
        boxShadow: '0 40px 120px rgba(8,6,24,0.6), inset 0 0 0 1px rgba(255,255,255,0.05)',
        overflow: 'hidden', padding: '22px 22px 0',
      }}>
        <div style={{ position: 'absolute', left: '50%', top: 16, transform: 'translateX(-50%)', width: 130, height: 30, borderRadius: 16, background: '#000' }} />
        ...
        <div style={{ position: 'relative', marginTop: 22, height: 388 }}>
          ...
          <div style={{
            ...
            {[0, 1, 2].map((i) => (
              <div key={i} style={{
                height: i === 0 ? 150 : 92, borderRadius: 20, background: 'rgba(255,255,255,0.045)',
          ...
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{
              ...
              <div style={{ width: 54, height: 54, borderRadius: 15, background: 'linear-gradient(140deg,#3ddc84,#1db765)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10, justifyContent: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <span style={{ fontFamily: C.mono, fontSize: 11, padding: '2px 9px', borderRadius: 8, background: C.violet, color: '#06331d', fontWeight: 600, letterSpacing: '0.06em' }}>NEW</span>
                </div>
        ...
        {flash > 0 && flashOut < 1 && (
          <div style={{
            ...
            border: `${clamp(6 - flash * 6, 1, 6)}px solid ${C.green}`, opacity: (1 - flash) * 0.9,
          }} />
        )}
        {flash > 0.2 && (
          <div style={{
        ...
        {toast > 0.01 && (
          <div style={{
            ...
            background: 'rgba(18,40,30,0.85)', border: '1px solid rgba(131,230,173,0.4)',
            boxShadow: '0 16px 40px rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)',
          }}>
            <span style={{ width: 26, height: 26, borderRadius: '50%', background: C.green, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
function Kicker() {
  ...
  return (
    <div style={{
      ...
      fontFamily: C.mono, fontSize: 15, letterSpacing: '0.32em', color: C.inkDim, textTransform: 'uppercase',
...
function Tagline() {
  ...
  const e1 = Easing.easeOutCubic(clamp((t - START) / 0.6, 0, 1));
  const e2 = Easing.easeOutCubic(clamp((t - START - 0.4) / 0.6, 0, 1));
  return (
    <div style={{ position: 'absolute', left: '50%', top: 150, transform: 'translateX(-50%)', textAlign: 'center', width: 1100 }}>
      <div style={{
        fontFamily: C.sans, fontWeight: 700, fontSize: 66, lineHeight: 1.08, color: C.ink,
        opacity: e1, transform: `translateY(${(1 - e1) * 22}px)`, letterSpacing: '-0.02em',
      ...
      <div style={{
        fontFamily: C.sans, fontWeight: 500, fontSize: 27, marginTop: 20, color: 'rgba(240,255,248,0.94)',
        opacity: e2, transform: `translateY(${(1 - e2) * 22}px)`, textShadow: '0 2px 20px rgba(3,30,18,0.65)',
      }}>
...
// ── responsive, self-driving stage (no editor chrome) ────────────────────────
export default function OtaMotion() {
  ...
  // Scale the fixed 1920×1080 layer to the container width.
  useLayoutEffect(() => {
  ...
  // Drive the timeline with rAF — but only while visible and if motion is
  // allowed. Under prefers-reduced-motion we hold a representative still.
  useEffect(() => {
    const reduce = window.matchMedia?.('(prefers-reduced-motion: reduce)').matches;
    if (reduce) { setTime(STATIC_TIME); return; }

    const el = wrapRef.current;
    let raf = null, last = null, visible = true;
    ...
    const start = () => { if (raf == null) { last = null; raf = requestAnimationFrame(step); } };
    const stop = () => { if (raf != null) { cancelAnimationFrame(raf); raf = null; } };

    const io = el ? new IntersectionObserver(([e]) => {
  ...
  const ctx = useMemo(() => time, [time]);

  return (
    <div
      ref={wrapRef}
      className="ota-motion"
      role="img"
      aria-label="Over-the-air delivery: Kotlin Compose is compiled to a signed .ktx bundle on the build server, streamed straight to a phone, and the app updates live to v2.4.1 without a Play Store release."
    >
      <div
        className="ota-motion-canvas"
        style={{
