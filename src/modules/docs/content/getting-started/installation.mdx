...
As of **`0.4.20-alpha`**, Ketoy no longer pins your project to a single Kotlin /
...
| Tool | Minimum | Notes |
|---|---|---|
| Kotlin | `2.0.0` | Any newer version (2.1, 2.2, 2.3) works, your toolchain never loads Ketoy's pinned compiler. |
...
Ketoy pins **Kotlin 2.0.21** internally for KBC emission, that governs only
...
1. Inserts `id("dev.ketoy.compiler") version "0.4.20-alpha"` into your
...
plugins {
    ...
    id("dev.ketoy.compiler") version "0.4.20-alpha"   // ← Ketoy plugin
...
dependencies {
    val ketoyBom = platform("dev.ketoy.vm:ketoy-bom:0.4.20-alpha")
