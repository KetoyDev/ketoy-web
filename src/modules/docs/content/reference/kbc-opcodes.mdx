...
tag-byte-prefixed payload (see [Compose Tokens](/docs/reference/compose-tokens) for
...
The full opcode list lives in `KBCOpcode.kt`, human-readable descriptors
in `KBCOpcodeInfo.kt`, and the typed `data class` variants in
`KBCInstruction.kt` (all in the `ketoy-bytecode` module).

Next: [`.ktx` Bundle Format →](/docs/reference/ktx-bundle-format)
