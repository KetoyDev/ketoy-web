...
the [Supported features](/features) page. This page is
...
[Kotlin Language guide](/docs/guides/kotlin-language)).
...
> **What isn't catalogued yet?** `OutlinedButton`, `TextButton`, `FAB`,
> `ExtendedFAB`, `OutlinedTextField`, `RadioButton`, `Slider`,
> `CircularProgressIndicator`, `LinearProgressIndicator`, `NavigationBar`,
> `TabRow`, `AlertDialog`, `BottomSheet`, `LazyVerticalGrid`,
> `HorizontalPager`. Their IDs are reserved in `KBCAdapterIds`; calling
> them today produces `UnregisteredComposable`. To add one, see
...
`@KetoyViewModel`. See [ViewModel guide](/docs/guides/viewmodel).
...
[Reusable components](/docs/guides/reusable-components) for the row
component pattern.
...
[Compose Token Registry](/docs/reference/compose-tokens):
...
See [Compile Errors](/docs/reference/compile-errors) for the full list
with sample messages.

Next: [ViewModel →](/docs/guides/viewmodel)
