...
export const _UNUSED_TRAVEL = `// Travel Booking Card
KColumn(
  modifier = kModifier(
    fillMaxSize = 1.0f,
    background = "#FFF0F4FF"
  ...
  // Hero Image Card
  KCard(
    modifier = kModifier(fillMaxWidth = 1.0f, height = 220),
    shape = KShapes.Rounded0,
    elevation = 0
  ) {
    KBox(
      ...
      KImage(
        source = kImageUrl("https://picsum.photos/400/220?random=5"),
        modifier = kModifier(fillMaxSize = 1.0f),
      ...
      KBox(
        modifier = kModifier(
          ...
          gradient = KGradients.linear(
            colors = listOf("#00000000", "#CC000000"),
            direction = "vertical"
          )
        ),
        contentAlignment = KAlignments.BottomCenter
  ...
  KColumn(
    ...
    KRow(
      ...
      KCard(
        ...
        containerColor = "#FF22C55E",
        elevation = 0
      ) {
        KText(
          text = "  50% OFF  ",
          modifier = kModifier(padding = kPadding(horizontal = 12, vertical = 6)),
          fontSize = 13,
          fontWeight = KFontWeights.Bold,
          color = "#FFFFFFFF"
        )
    ...
    KButton(containerColor = "#FF2563EB") {
      KRow(
        modifier = kModifier(
          padding = kPadding(horizontal = 20, vertical = 4)
        ),
